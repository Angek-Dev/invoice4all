...
export const InvoiceDocument = ({ payload }: { payload: InvoicePayload }) => {
    ...
    const itemsTotal = (payload.items || []).reduce((sum, item) => sum + (item.cost || 0) * (item.quantity || 0), 0);
    
    const dynamicAdjustmentsTotal = (payload.adjustments || []).reduce((sum, adj) => {
        const amountDollars = (adj?.amountCents || 0) / 100;
        return adj?.type === "addition" ? sum + amountDollars : sum - amountDollars;
    }, 0);

    const total = Math.max(0, itemsTotal + dynamicAdjustmentsTotal);
    
    const dollars = Math.floor(total);
    const cents = Math.round((total - dollars) * 100);

    let totalInWords = "Zero";
    try {
        if (dollars > 0) {
            totalInWords = toWords(dollars);
        }
        if (cents > 0) {
            totalInWords = dollars > 0 
                ? `${totalInWords} and ${cents.toString().padStart(2, "0")} cents`
                : `${cents.toString().padStart(2, "0")} cents`;
        }
        totalInWords = totalInWords.charAt(0).toUpperCase() + totalInWords.slice(1);
    } catch (e) {
        totalInWords = "Amount Render Error";
    }
    ...
    return (
        <Page size="A4" style={styles.page}>
                    ...
                    <View key={idx} style={styles.tableRow}>
                        ...
                        <View style={{ flex: 30, flexDirection: "column" }}>
                            ...
                            {item.stops && item.stops.map((stop, sIdx) => {
                                ...
                                if (stop.datetime) {
                                    try {
                                        dateString += ` - ${new Date(stop.datetime).toLocaleDateString("en-US")}`;
                                    } catch(e){}
                                }
                                if (stop.datetime2) {
                                    try {
                                        dateString += ` - ${new Date(stop.datetime2).toLocaleDateString("en-US")}`;
                                    } catch(e){}
                                }

                                const locationStr = `${stop.city || ""}${stop.state ? `, ${stop.state}` : ""}${stop.zip ? `, ${stop.zip}` : ""}`;
                                const headerText = `${stop.type || ""}${dateString}`;
                        ...
                        <Text style={{ flex: 3 }}>{item.quantity || 0}</Text>
                        <Text style={styles.cell}>${(item.cost || 0).toFixed(2)}</Text>
                        <Text style={styles.cell}>${((item.cost || 0) * (item.quantity || 0)).toFixed(2)}</Text>
                    </View>
                ))}
            </View>
            ...
            <View style={{ marginTop: 10, width: 225, alignSelf: "flex-end" }}>
                ...
                {Array.isArray(payload.adjustments) && payload.adjustments.map((adj) => {
                    if (!adj) return null;
                    const displayAmount = (adj.amountCents || 0) / 100;
